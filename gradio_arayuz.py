...
def run_and_parse(model_name, operation):
    ...
    if model_name == "EfficientAD":
        ...
        image_dir = "/content/anomaly_detection/results/EfficientAd/wood/latest/images"
    ...
    log_file_path = f"/content/anomaly_detection/logs/{model_name}_log.txt"
    ...
    if operation == "Test":
        if model_name == "EfficientAD":
            if os.path.exists(image_dir):
                image_paths = glob(f"{image_dir}/*.png") + glob(f"{image_dir}/*.jpg")
        else:
        ...
        if len(image_paths) > 8:
            image_paths = random.sample(image_paths, 8)
